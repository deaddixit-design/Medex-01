...
// Database Migrations for BMLT Case Studies
try {
  db.prepare("ALTER TABLE bmlt_case_studies ADD COLUMN normal_params TEXT").run();
} catch (e) {}
try {
  db.prepare("ALTER TABLE bmlt_case_studies ADD COLUMN explanation TEXT").run();
} catch (e) {}
...
`;
  ...
  app.get('/api/bmlt/mcqs', (req, res) => {
    try {
      const mcqs = db.prepare('SELECT * FROM bmlt_mcqs ORDER BY id DESC').all() as any[];
  ...
  app.post('/api/bmlt/mcqs', requireAdmin, (req, res) => {
    const { 
      question, 
  ...
  app.put('/api/bmlt/mcqs/:id', requireAdmin, (req, res) => {
    const { 
      question, 
      option_a, option_b, option_c, option_d, options, 
      correct_option, correct, 
  ...
  app.delete('/api/bmlt/mcqs/:id', requireAdmin, (req, res) => {
    try {
  ...
  app.get('/api/bmlt/cases', (req, res) => {
    try {
      const cases = db.prepare('SELECT * FROM bmlt_case_studies ORDER BY id DESC').all() as any[];
  ...
  app.post('/api/bmlt/cases', requireAdmin, (req, res) => {
    const { 
  ...
  app.put('/api/bmlt/cases/:id', requireAdmin, (req, res) => {
    const { 
      title, presentation, scenario, type, question, 
      option_a, option_b, option_c, option_d, options,
      correct_option, correct, correct_guidelines, explanation,
  ...
  app.delete('/api/bmlt/cases/:id', requireAdmin, (req, res) => {
    try {
  ...
  app.post('/api/bmlt/slides', requireAdmin, (req, res) => {
  ...
  app.put('/api/bmlt/slides/:id', requireAdmin, (req, res) => {
    const { name, description, target_cell, targetCell, hint, image_url, imageUrl, fact, options, hotspots } = req.body;
    const final_target_cell = target_cell || targetCell;
    const final_image_url = image_url || imageUrl;
    if (!name || !description || !final_target_cell || !final_image_url || !options) {
  ...
  app.delete('/api/bmlt/slides/:id', requireAdmin, (req, res) => {
    try {
  ...
  app.get('/api/bmlt/lab-params', (req, res) => {
    try {
      const params = db.prepare('SELECT * FROM bmlt_lab_params ORDER BY id ASC').all() as any[];
  ...
  app.post('/api/bmlt/lab-params', requireAdmin, (req, res) => {
    const { 
      name, unit, 
  ...
  app.put('/api/bmlt/lab-params/:id', requireAdmin, (req, res) => {
    const { 
      name, unit, 
      normal_min_male, normalMinMale, 
      normal_max_male, normalMaxMale, 
  ...
  app.delete('/api/bmlt/lab-params/:id', requireAdmin, (req, res) => {
    try {
  ...
  app.get('/api/admin/backup-db', requireAdminOnly, (req, res) => {
    try {
      if (!fs.existsSync(dbPath)) {
        return res.status(404).json({ error: 'Database file not found' });
      }
      res.download(dbPath, 'campus_pulse_backup.db');
    } catch (err: any) {
      console.error('Database backup failed:', err);
      res.status(500).json({ error: err.message || 'Failed to backup database' });
    }
  });

  app.post('/api/admin/restore-db', requireAdminOnly, upload.single('file'), (req, res) => {
    try {
      if (!req.file) {
      ...
      // Basic validation: verify the file is a SQLite database
      ...
      if (buffer.toString('utf-8', 0, 15) !== 'SQLite format 3') {
        ...
        return res.status(400).json({ error: 'Invalid file format. Please upload a valid SQLite database file.' });
      }

      console.log('Restoring database. Closing current DB connection...');
      db.close();
      ...
      console.log('Re-opening DB connection...');
      ...
      try {
        ...
        console.error('Failed to re-open database after error:', reopenErr);
      }
